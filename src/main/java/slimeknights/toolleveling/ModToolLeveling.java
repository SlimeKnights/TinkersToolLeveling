...
public class ModToolLeveling extends ModifierTrait{
    ...
    public void afterHit(ItemStack tool, EntityLivingBase player, EntityLivingBase target, float damageDealt, boolean wasCritical, boolean wasHit){
        if(!target.getEntityWorld().isRemote && wasHit && player instanceof EntityPlayer){
            ...
            if(!target.isEntityAlive()){
                ...
                IDamageXp capability = target.getCapability(CapabilityDamageXp.CAPABILITY, null);
                if(capability != null){
                    capability.addDamageFromTool(damageDealt, tool, entityPlayer);
                }
            }
        }
    }
  ...
  /* XP Handling */
    ...
    public int getXpForLevelup(int level, ItemStack tool){
        ...
        return (int)((float)getXpForLevelup(level - 1, tool) * Config.levelMultiplier);
    }
